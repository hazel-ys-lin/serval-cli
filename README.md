...
**Status:** v0.3.1 — Phase 0 (lite-native CLI scaffold) and Phase 1 (CLI primary interface) complete; v0.3.1 ships prebuilt binaries via [`cargo-dist`](https://github.com/axodotdev/cargo-dist). Source-agnostic ingestion (OpenAPI 3.x / AsyncAPI) is on the roadmap; today the CLI consumes Gherkin only.
...
```sh
cargo install --git https://github.com/hazel-ys-lin/serval-cli --tag v0.3.1
```
